...
/**
 * Servlet implementation class DeleteStar
 */
@WebServlet("/DeleteStar")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {  
	        ...
	        if(loginRs.next()) {  
                ...
                if (userid != Integer.valueOf(FRIENDID)) {
    				String deleteSql="delete from star where userid=? and friendid=?";
    	        	PreparedStatement deletePreStmt = con.prepareStatement(deleteSql);//句柄
    	        	deletePreStmt.setInt(1, userid);  
    	        	deletePreStmt.setInt(2, Integer.valueOf(FRIENDID));
    	        	int deleteRs=deletePreStmt.executeUpdate(); 
    	        	out.print("成功取消关注"+deleteRs+"个好友");					
				}
	        }
        }  
        catch(Exception ex) {  
